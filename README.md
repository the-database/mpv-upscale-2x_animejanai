...
Most HD anime is [not produced in native 1080p resolution](https://guide.encode.moe/encoding/descaling.html), but some resolution between 720p and 1080p. When the video is prepared for distribution, it is scaled to 1080p resolution which results in scaling artifacts and a blurry image. In training these models, the goal was to correct these scaling and blur issues while upscaling to produce a result that appears as if the anime was originally mastered in 4k resolution.
